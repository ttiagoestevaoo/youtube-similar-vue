...
export class YoutubeVideosService {
  static async list(
    ...
    const { items } = (await http.get(`${BASE_URL}/videos?${queryString}`))
      .data;
  ...
  static async search(
    ...
    const queryString = `part=snippet&key=${GOOGLE_API_KEY}&maxResults=${maxResults}&q=${description}&regionCode=BR&type=video`;

    const { items } = (await http.get(`${BASE_URL}/search?${queryString}`))
      .data;
